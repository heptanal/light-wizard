...
fn configure<R: BufRead, W: Write>(
    ...
    config.visualizer.attack_ms = prompt_value(
        ...
        "How quickly smoothed brightness rises toward a louder level. Small values make transients snap; larger values create gentler fades. Must be positive; 20–100 ms is a useful range.",
    ...
    config.visualizer.release_ms = prompt_value(
        ...
        "How quickly brightness falls after audio becomes quieter. Longer release reduces flicker and produces flowing fades. Must be positive; 150–500 ms is a useful range.",
    ...
    config.visualizer.beat_boost = prompt_value(
        ...
        "Temporary 0–1 intensity added to a detected beat before brightness smoothing. 0 disables brightness emphasis; 0.22 adds a noticeable accent.",
    ...
    config.visualizer.pulse_on_beat = prompt_value(
        ...
        "Send the firmware-native pulse effect in addition to normal color and brightness frames. It can create sharper accents but its appearance varies by light model, so it is disabled by default.",
